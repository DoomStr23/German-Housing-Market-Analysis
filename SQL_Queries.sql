...
SELECT regio1_clean AS state_or_city,
       COUNT(*) AS listings,
...
-- View: state/city summary
CREATE OR REPLACE VIEW vw_city_rent_summary AS
SELECT
  regio1_clean AS city,
  COUNT(*) AS listings,
  ROUND(AVG(totalrent)::numeric, 2) AS avg_rent,
  ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY totalrent)::numeric, 2) AS median_rent,
...
-- Top 20 most expensive cities (by average rent)
SELECT city, avg_rent, listings
FROM vw_city_rent_summary
ORDER BY avg_rent DESC
LIMIT 20;
...
-- Price trend (if you have date -> monthly)
SELECT date_trunc('month', to_date(date, 'MonYY'))::date AS month, 
       ROUND(AVG(totalrent)::numeric,2) AS avg_rent
FROM german_rent_data
WHERE date IS NOT NULL -- ensure proper date parsing
GROUP BY month
ORDER BY month;
...
DROP TABLE IF EXISTS cleaned_rent_for_powerbi;

CREATE TABLE cleaned_rent_for_powerbi AS
SELECT
  scoutid,
  regio1_clean AS city,
  plz_text,
  ...
  garden,
  lastrefurbish,
  energyEfficiencyClass AS energy_class,
  date
FROM german_rent_data
WHERE totalrent IS NOT NULL AND livingspace IS NOT NULL;

CREATE INDEX idx_powerbi_city ON cleaned_rent_for_powerbi (city);
CREATE INDEX idx_powerbi_plz ON cleaned_rent_for_powerbi (plz_text);
